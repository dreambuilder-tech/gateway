...
func (c *Svr) orderChatJoin(s *melody.Session, memberID int64, payload json.RawMessage) (*wsproto.PushMsg, error) {
	...
	resp, err := im_rpcx.CheckSessionAccess(s.Request.Context(), &contracts.CheckSessionAccessReq{
		SessionID: req.SessionID,
		MemberID:  memberID,
	})
	if err != nil {
	...
	if !resp.Allow {
		ack.Code = wsproto.CodePermissionDenied
		return ack, fmt.Errorf("member permission denied")
	}
...
func (c *Svr) messageSendAck(s *melody.Session, memberID int64, payload json.RawMessage) (*wsproto.PushMsg, error) {
	...
	if code := sendLimit(s, req.SessionID); code != wsproto.CodeSuccess {
		r.Code = code
		return r, fmt.Errorf("hit the limit. code: %d", code)
	}

	// 发送内容检查
	if code := verifyBlocks(memberID, req.Content); code != wsproto.CodeSuccess {
		r.Code = code
